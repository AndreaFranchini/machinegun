...
function countVictims(ground, rps, reportPath) {
  const count = {
    status100: 0,
    status200: 0,
    status300: 0,
  ...
  ground.forEach((victim) => {
    switch (victim.status.toString()[0]) {
      ...
      case '5': count.status500 += 1; break;
      default: break;
    }
    ...
    if (victimDateSeconds === bunchDateSeconds) {
      ...
      fs.appendFileSync(reportPath, `${bunchTime};${bunchMinLatency};${bunchMediumLatency};${bunchMaxLatency}\n`);
      bunch = [];
    }
  });
  ...
  return {
    ...
    requestLandedPerSecond: Number((ground.length / time.duration.fire).toFixed(3)),
    requestLanded: ground.length,
    count,
